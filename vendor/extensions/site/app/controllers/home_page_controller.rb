class HomePageController < Spree::BaseController
  ...
  def show
    if @current_domain != @site.code
      ...
      if @site.code == "pwb"
        @best_selling_cats = Product.find(Taxon.find([9998,3002]).map(&:product_ids).inject {|x, y| x &  y }).find_all {|p| !p.deleted? && p.available_on < Time.now}
        @best_selling_dogs = Product.find(Taxon.find([9998,3001]).map(&:product_ids).inject {|x, y| x &  y }).find_all {|p| !p.deleted? && p.available_on < Time.now}
      else
